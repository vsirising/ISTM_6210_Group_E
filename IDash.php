...
function getPosts()
{
    ...
    $posts[9] = $_POST['InstructorName'];
    $posts[10] = $_POST['specialty'];
...
$searchStmt =$db->prepare('SELECT * FROM account WHERE InstructorID = :InstructorID');
$searchStmt->bindParam(':InstructorID', $getInstructorID, PDO::PARAM_STR);
$searchStmt->execute();
...
$searchStmt2 =$db->prepare('SELECT * FROM instructor WHERE InstructorID = :InstructorID');
$searchStmt2->bindParam(':InstructorID', $getInstructorID, PDO::PARAM_STR);
$searchStmt2->execute();
...
{
    $data = getPosts();
    if(empty($data[0]) || empty($data[1]) || empty($data[2]) || empty($data[3]) || empty($data[4]) || empty($data[5]) || empty($data[6]) || empty($data[7]) || empty($data[8]) || empty($data[9]) || empty($data[10]))
    {
        ...
        $updateStmt = $db->prepare('UPDATE account SET Email = :email, First_Name = :firstname, Last_Name = :lastname, Phone_Number = :phone, Street_Address = :address, City = :city, State = :states, ZipCode = :zipcode WHERE InstructorID = :InstructorID');
        $updateStmt->execute(array(
         ...
         $updateStmt2 = $db->prepare('UPDATE instructor SET InstructorName = :InstructorName, Specialty = :Specialty WHERE InstructorID = :InstructorID');
         $updateStmt2->execute(array(
                    'InstructorID' => $data[0],
                    'InstructorName' => $data[9],
                    'Specialty' => $data[10],

        ));
         
            header("Location: IDash.php"); /* Redirect browser */
	                exit();
					...
					<div class="row">
						<div class="col-md-12 text-center">
                            ...
                            <h1 style="color:white">
                                    <?php if(isset($_SESSION['instructor_login'])) { ?>
						...
						<div class="row workouts">
							...
							<div class="col-md-6">
								<div class="Workouts" class="cover" style="background:url('img/table-4.jpg'); background-size:cover;">
                                    ...
                                    <br>
                                    <br>
									<a href="Icardio.php"  class="btn btn-blue-fill">View Upload Page</a> 
								</div>
							...
							<div class="col-md-6">
								<div class="Workouts" class="cover" style="background:url('img/table-5.jpg'); background-size:cover;">
                                    ...
                                    <br>
                                    <br>
									<a href="Iyoga.php" class="btn btn-blue-fill">View Upload Page</a>
								</div>
							</div>
							...
							<div class="wpb_column vc_column_container col-md-6">
								<div class="Workouts" class="cover" style="background:url('img/table-6.jpg'); background-size:cover;">
                                    ...
                                    <br>
                                    <br>
									<a href="Istrength.php"  class="btn btn-blue-fill">View Upload Page</a>
								</div>
							</div>
		</div>	
        </div>
...
<!-- User Modal Display -->
			...
			<div class="modal-content modal-popup">
				...
				<form action="IDash.php" method = "post" class="popup-form">
                    
                    <input type="text" name = "InstructorID" class="form-control form-white" value="<?php echo $getInstructorID; ?>" readonly>
                    ...
                    <input type="text" name = "InstructorName" class="form-control form-white" value="<?php echo $instructorname; ?>" placeholder="Nick Name">
                    <input type="text" name = "specialty" class="form-control form-white" value="<?php echo $specialty; ?>" placeholder="Specialty">
